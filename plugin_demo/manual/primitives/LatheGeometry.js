...
Page({
    ...
    createMesh() {
        ...
        var mesh = new THREE.Mesh(new THREE.LatheGeometry(
            points,
            this.setting.segments,
            this.setting.phiStart,
            this.setting.phiLength,
        ), material);
    ...
    async run(canvas) {
        ...
        function createPanel() {
            ...
            folder2.add({
                name: "phiStart",
                phiStart: that.setting.phiStart
            }, 'phiStart', 0.0, Math.PI * 2, 0.01).onChange((value) => {
                that.setting.phiStart = value;
                that.createMesh();
            });
            folder2.add({
                name: "phiLength",
                phiLength: that.setting.phiLength
            }, 'phiLength', 0.0, Math.PI * 2, 0.01).onChange((value) => {
                that.setting.phiLength = value;
                that.createMesh();
            });
            //
            folder3.add({
            ...
            }, 'segments', 0, 10, 1).onChange((value) => {
