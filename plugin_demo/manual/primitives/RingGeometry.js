...
Page({
    ...
    createMesh() {
        ...
        var mesh = new THREE.Mesh(new THREE.RingGeometry(
            this.setting.innerRadius,
            this.setting.outerRadius,
            this.setting.thetaSegments,
            this.setting.phiSegments,
    ...
    async run(canvas) {
        ...
        function createPanel() {
            ...
            folder2.add({
                name: "innerRadius",
                innerRadius: that.setting.innerRadius
            }, 'innerRadius', 0.0, 10, 0.01).onChange((value) => {
                that.setting.innerRadius = value;
                that.createMesh();
            });
            folder2.add({
                name: "outerRadius",
                outerRadius: that.setting.outerRadius
            }, 'outerRadius', 0.0, 10, 0.01).onChange((value) => {
                that.setting.outerRadius = value;
                that.createMesh();
            });
            //
            folder3.add({
            ...
            folder4.add({
                name: "thetaSegments",
                thetaSegments: that.setting.thetaSegments
            }, 'thetaSegments', 1, 24, 1).onChange((value) => {
                that.setting.thetaSegments = value;
                that.createMesh();
            });
            folder4.add({
                name: "phiSegments",
                phiSegments: that.setting.phiSegments
            }, 'phiSegments', 1, 24, 1).onChange((value) => {
                that.setting.phiSegments = value;
                that.createMesh();
            });
        }
        createPanel()
