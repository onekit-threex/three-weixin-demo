...
export default class Controller {
  static findPATH(that) {
    if (!that.parent) {
      return ""
    }
    var parent = that.parent
    var index = null
    for (var i = 0; i < that.parent.children.length; i++) {
      const item = that.parent.children[i]
      if (item.isGUI) {
        if (item._title == that._title) {
          index = i
          break
        }
      } else {
        if (item.property == that.property) {
          index = i
          break
        }
      }
    }
    return `${parent.PATH}children[${index}].`
  }
  get PATH() {
  ...
  constructor(parent, object, property, className, widgetTag = 'div') {
    ...
    var children = parent._findData(`${parent.PATH}children`)
    children.push({
      type: className
    })
    var data = parent.UC.data
    data[`${parent.PATH}children`] = children
    parent.UC.setData(data)
    ////////////////////////////////////////////
    /**
   ...
   * @returns {this}
   * @example
   * const controller = gui.add_( object_, 'property' );
   *
  ...
  disable(disabled = true) {
    ...
    this._render("disabled", disabled)
    return this;
   ...
   * // safe usage
   *
   * gui.add_( object1, 'property' ).options( [ 'a', 'b', 'c' ] );
   * gui.add_( object2, 'property' );
   *
   * // danger
   *
   * const c = gui.add_( object1, 'property' );
   * gui.add_( object2, 'property' );
   *
   * c.options( [ 'a', 'b', 'c' ] );
  ...
  destroy() {
    ...
    var key = `${this.parent.PATH}children`
    const list = this.parent.UC.data[key];
    //for(const i=index;i<list.length;i++)
    list.splice(index, 1)
    const data = {}
    data[key] = list
    this.parent.UC.setData(data)
    ////////////////////////////////////////////////////////
  ...
  _render(key_, value) {
    const data = this.parent.UC.data
